...
// Check if user is connected to Ethereum and get the account
...
const createScene = () => {
    ...
    spinButton.addEventListener("click", () => {
        ...
        // Call blockchain function before spinning
        recordSpinOnBlockchain().then(() => {
...
// Blockchain function to record spin event
async function recordSpinOnBlockchain() {
    try {
        // Assuming the contract has a function to record spins
        const receipt = await contract.methods.recordSpin().send({ from: userAccount });
        console.log('Spin recorded on blockchain:', receipt);
    } catch (error) {
        console.error('Error recording spin on blockchain:', error);
    }
}
